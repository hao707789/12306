...
//    		res.getAllHeader().PrintlnAllHeaderInfo();	//打印头信息
    		...
    		String result = HttpUtils.outHtml(res.getBody());
...
//			rs.getAllHeader().PrintlnAllHeaderInfo();	//打印头信息
			...
			String code = HttpUtils.outCode(rs.getBody(),0,0);	//普通验证码获取
...
//			loginrs.getAllHeader().PrintlnAllHeaderInfo();	//打印头信息
			...
			if (loginrs.getEntity().getStaus()==200) {
				String body = HttpUtils.outHtml(loginrs.getBody());
...
//				rs2.getAllHeader().PrintlnAllHeaderInfo();	//打印头信息
				...
				HttpUtils.PrintHtmlByLine(rs2.getBody());
			...
			while (true){
				...
				String body = HttpUtils.outHtmlByLine(res2.getBody());
